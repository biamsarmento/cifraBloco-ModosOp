...
#def inv_mix_columns(state):
#    """Aplica a operação inversa de MixColumns no estado AES."""
#    new_state = [[0] * 4 for _ in range(4)]  # Cria uma matriz temporária 4x4

#    for col in range(4):  # Percorre cada coluna
#        new_state[0][col] = (
#            mul_by_0E(state[0][col]) ^
#            mul_by_0B(state[1][col]) ^
#            mul_by_0D(state[2][col]) ^
#            mul_by_09(state[3][col])
#        )
#        new_state[1][col] = (
#            mul_by_09(state[0][col]) ^
#            mul_by_0E(state[1][col]) ^
#            mul_by_0B(state[2][col]) ^
#            mul_by_0D(state[3][col])
#        )
#        new_state[2][col] = (
#            mul_by_0D(state[0][col]) ^
#            mul_by_09(state[1][col]) ^
#            mul_by_0E(state[2][col]) ^
#            mul_by_0B(state[3][col])
#        )
#        new_state[3][col] = (
#            mul_by_0B(state[0][col]) ^
#            mul_by_0D(state[1][col]) ^
#            mul_by_09(state[2][col]) ^
#            mul_by_0E(state[3][col])
#        )
...
# Função de InvMixColumns -- made by Bia
#def inv_mix_columns(state):
#    temp = state
...
#-----made by gabi
def xtime(x):
    """Multiplica um byte por x no campo GF(2^8)."""
    return ((x << 1) & 0xFF) ^ (0x1B if (x & 0x80) else 0)

def mul_by_09(x):
    """Multiplica um byte por 0x09 no campo GF(2^8)."""
    return xtime(xtime(xtime(x))) ^ x

def mul_by_0b(x):
    """Multiplica um byte por 0x0B no campo GF(2^8)."""
    return xtime(xtime(xtime(x))) ^ xtime(x) ^ x

def mul_by_0d(x):
    """Multiplica um byte por 0x0D no campo GF(2^8)."""
    return xtime(xtime(xtime(x))) ^ xtime(xtime(x)) ^ x

def mul_by_0e(x):
    """Multiplica um byte por 0x0E no campo GF(2^8)."""
    return xtime(xtime(xtime(x))) ^ xtime(xtime(x)) ^ xtime(x)

def inv_mix_columns(state):
    """Aplica a operação inversa de MixColumns no estado AES."""
    # Cria uma cópia da matriz original para evitar interferência nos cálculos
    original_state = [row[:] for row in state]
    new_state = [[0] * 4 for _ in range(4)]  # Matriz temporária para armazenar os resultados

    # Processa cada coluna isoladamente
    for col in range(4):
        a0 = original_state[0][col]
        a1 = original_state[1][col]
        a2 = original_state[2][col]
        a3 = original_state[3][col]

        # Calcula os valores resultantes para a coluna usando as funções de multiplicação
        new_state[0][col] = mul_by_0e(a0) ^ mul_by_0b(a1) ^ mul_by_0d(a2) ^ mul_by_09(a3)
        new_state[1][col] = mul_by_09(a0) ^ mul_by_0e(a1) ^ mul_by_0b(a2) ^ mul_by_0d(a3)
        new_state[2][col] = mul_by_0d(a0) ^ mul_by_09(a1) ^ mul_by_0e(a2) ^ mul_by_0b(a3)
        new_state[3][col] = mul_by_0b(a0) ^ mul_by_0d(a1) ^ mul_by_09(a2) ^ mul_by_0e(a3)
...
def aes_decrypt(ciphertext, key):
    ...
    # Estado inicial
    state = ciphertext
    print("State 0 (ciphertext): ", format_state_hex(state))

    # Etapa inicial: Adiciona a chave da última rodada
    state = inv_add_round_key(state, round_keys[0])
    print("After initial inv_add_round_key: ", format_state_hex(state))
    ...
    for round in range(1, 10):
        ...
        state = inv_shift_rows(state)
        print(f"round[{round}].inv_shift_rows: {format_state_hex(state)}")

        state = inv_sub_bytes(state)
        print(f"round[{round}].inv_sub_bytes: {format_state_hex(state)}")

        state = inv_add_round_key(state, round_keys[round])
        print(f"round[{round}].inv_add_round_keys: {format_state_hex(state)}")

        state = inv_mix_columns(state)
        print(f"round[{round}].inv_mix_columns: {format_state_hex(state)}")

    # Rodada final (sem InvMixColumns)
    state = inv_shift_rows(state)
    print("Final round.inv_shift_rows: ", format_state_hex(state))

    state = inv_sub_bytes(state)
    print("Final round.inv_sub_bytes: ", format_state_hex(state))

    state = inv_add_round_key(state, round_keys[-1])
    print("Final round.inv_add_round_keys: ", format_state_hex(state))
